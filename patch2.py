...
content = content.replace(
    "background: linear-gradient(135deg, var(--forest) 0%, var(--green-dark) 50%, var(--deep) 100%);",
    "background: transparent;"
)
content = content.replace(
    "radial-gradient(circle at 30% 40%, rgba(61, 139, 92, 0.3) 0%, transparent 50%),",
    "radial-gradient(circle at 30% 40%, transparent 0%, transparent 50%),"
)
content = content.replace(
    "radial-gradient(circle at 70% 70%, rgba(74, 103, 65, 0.2) 0%, transparent 40%);",
    "radial-gradient(circle at 70% 70%, transparent 0%, transparent 40%);"
)
...
script_insert = """
  <script>
    ...
    (function init3D() {
      ...
      renderer.setClearColor(0x080b07, 1);
      ...
      function onWindowResize() {
        ...
        if(window.innerWidth <= 1024) {
          gsap.set(terrariumGroup.position, { x: 0 });
        }
      }
